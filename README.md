...
where `other` can be a single observation, a collection of observations (an iterator), or
...
A `M`-order sample statistics object `stat` stores the number of observations and `M`
*moments* `μₖ` (for `k=1` to `M`) given by `stat[k]` and which, for `n` observations, `x₁`,
`x₂`, ..., `xₙ`, are defined by:
...
is the type `T` or `eltype(iter)` in the above examples converted to floating-point because
statistical moments can only be memorized as floating-point numbers. As can be noted, a
*trait* does only depend on the type of the object.
...
Comparison with the *standard package* `Statistics` in Julia and the
[`StatsBase`](https://github.com/JuliaStats/StatsBase.jl) package in the *general registry*:

- For arrays and when vectorization is applicable, `SampleStat` is faster to compute the
  mean and variance (or standard deviation) because .

Comparison with [`OnlineStats`](https://github.com/joshday/OnlineStats.jl):

- For now, `SampleStats` has a simple and limited objective: compute or update the
  statistical moments of observations. `OnlineStats` offers many other possibilities.
...
- Computing sample statistics with `SampleStats` is as fast or faster than with
  `OnlineStats`.
...
Comparison with
[`MultiVariateOnlineStatistics`](https://github.com/emmt/MultiVariateOnlineStatistics.jl):

- `MultiVariateOnlineStatistics` computes the sample statistics of multi-variate
  observations, an observation being specified by an array. In `SampleStat`, an observation
  is a number.

- `MultiVariateOnlineStatistics` is limited to 2nd order (variance) statistics. `SampleStat`
  has no such limitation.

- `MultiVariateOnlineStatistics` could use `SampleStat` to compute the moments and relax the
  above limitations.
...
- `OnlineSampleStatistics` uses mutable objects.
